...
- `webhook_inbox` — id, `provider TEXT`, `provider_event_id TEXT`, payload jsonb, received_at, processed_at NULL. Dedup is a composite unique index on `(provider, provider_event_id)` — keeping the `provider` column in the key prevents collision when a second adapter (Twilio / Vapi / Retell) lands using its own event-id namespace.
