...
- **Webhook**: ack-then-process via `webhook_inbox` with `provider_event_id UNIQUE`. Ingest-route order is strict: (1) INSERT the inbox row inside `async with conn.transaction():` and let the transaction commit, (2) THEN — outside the transaction scope — fire a tracked `asyncio.create_task(process_pending_inbox())` so the processor runs immediately, (3) return `200`. Spawning the task before commit would race: the processor could dequeue before the new row is visible to its snapshot, silently no-op, and leave the row for the periodic sweep. A periodic sweep task also runs (every `SCHEDULER_SAFETY_NET_SECONDS`) to pick up rows orphaned by a crash between commit and task spawn.
