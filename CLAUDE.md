...
- **Webhook**: ack-then-process via `webhook_inbox` with `provider_event_id UNIQUE`. After the inbox INSERT, the ingest route fires a tracked `asyncio.create_task(process_pending_inbox())` so the processor runs immediately — no polling gap between webhook arrival and state transition. A periodic sweep task also runs (every `SCHEDULER_SAFETY_NET_SECONDS`) to pick up rows that slipped past (e.g. ingest crashed between INSERT and task spawn).
