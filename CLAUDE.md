...
2. **Concurrency gate** applied to every candidate (retry or new): skip any campaign whose **in-flight count ≥ `max_concurrent`**. In-flight = rows with `status IN ('DIALING', 'IN_PROGRESS')`. `RETRY_PENDING` and `QUEUED` rows are waiting, not active, and do NOT count. Gate is evaluated ONCE per tick and applies to both the retry sweep and the round-robin pick — a retry-due call on a capped campaign waits exactly like a new call would. **Atomicity**: in the single-process topology, the scheduler tick runs sequentially — count-then-claim is race-free by sequencing. Horizontal scale (multi-process / multi-replica) requires `pg_try_advisory_xact_lock(hashtextextended(campaign_id::text, 0))` wrapped around the gate + claim. Use the 64-bit `hashtextextended`, not 32-bit `hashtext` — the advisory-lock key space is 64-bit, and the narrower hash would silently serialize unrelated campaigns on collision. Flagged as README future-work alongside the general replica story.
3. **Retry sweep**: among campaigns that cleared the concurrency gate, if any has a `RETRY_PENDING` call whose `next_attempt_at` has passed, dispatch a retry first (system-level retries-before-new). When **multiple** campaigns have retries due, apply the SAME RR cursor as step 4 so no single campaign's retry backlog monopolizes the retry-sweep slot. Retries beat new calls system-wide; no campaign starves another during a retry storm — the assignment's "fairness" qualifier is preserved.
...
- **Campaign terminal trigger**: every call's terminal transition (→ `COMPLETED` / `FAILED`) runs, in the SAME transaction, a parent-campaign check. If the campaign has zero calls in `{QUEUED, DIALING, IN_PROGRESS, RETRY_PENDING}`, transition the campaign itself to `COMPLETED` (at least one call succeeded) or `FAILED` (every terminal call is `FAILED`). Keeps campaign status accurate without a separate sweep.
...
- `call.attempt_epoch` (int) increments **exactly once per dial attempt**, at two well-defined sites: (a) the claim primitive (`QUEUED → DIALING` transition), and (b) the stuck-reclaim branch (`DIALING → QUEUED` with bumped epoch). Retry requeue (`FAILED → RETRY_PENDING → QUEUED`) does NOT bump — the next claim handles it. Terminal transitions (to `COMPLETED`, `FAILED`, `NO_ANSWER`, `BUSY`) do NOT bump. This keeps the provider idempotency key `f"{call_id}:{attempt_epoch}"` distinct for exactly one provider-facing dial per attempt.
- **Stuck reclaim** runs on a background sweep, NEVER on the dispatch critical path. When `DIALING` exceeds `MAX_CALL_DURATION_SECONDS + 30s` (global setting in `config.py`; per-campaign override deferred as README future-work), FIRST call `provider.get_status(call_id)` as a **best-effort confirm** (configurable timeout `STUCK_RECLAIM_GET_STATUS_TIMEOUT_SECONDS`, bounded well under the sweep interval so a provider brownout doesn't degrade the sweep). Correctness does NOT depend on bypassing provider-side caches — it depends on the grace window being wider than any plausible provider cache TTL plus the epoch CAS at apply time. On timeout or error, treat the result as unknown and proceed to reclaim.
...
- **Webhook**: ack-then-process via `webhook_inbox` with `provider_event_id UNIQUE`. Ingest-route order is strict: (1) INSERT the inbox row inside `async with conn.transaction():` and let the transaction commit, (2) THEN — outside the transaction scope — fire `asyncio.create_task(process_pending_inbox())` AND **add the returned task handle to the module-level tracked-task set** (see `backend-conventions`) so Python's GC can never collect the task before it runs, (3) return `200`. Spawning the task before commit would race: the processor could dequeue before the new row is visible to its snapshot, silently no-op, and leave the row for the periodic sweep. A periodic sweep task also runs (every `SCHEDULER_SAFETY_NET_SECONDS`, tight-bounded so worst-case latency is acceptable) to pick up rows orphaned by a crash between commit and task spawn OR by event-loop saturation that delayed the per-request task.
...
- **Three asyncpg pools** (api / scheduler / webhook) with explicit sizing in `backend-conventions` skill. Webhook burst can't starve API or scheduler; `/audit` reads go to `api_pool`, never `scheduler_pool`.
- **No global CPS throttle.** Per-campaign `max_concurrent` is the only rate control — the assignment specifies "maximum concurrent calls ... within a campaign," no global limit. Adding a token bucket would be spec-creep. Documented as future work if a real provider enforces a global CPS on the account.
